...
function CashflowPage() {
  ...
  const handleDelete = (id: string) => {
    // Optimistic UI — already instant via local state
        ...
        <form onSubmit={submit} className="grid grid-cols-2 md:grid-cols-6 gap-3">
          <Field className="col-span-2 md:col-span-2" label="Date">
            <Input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })}
              className="bg-input/40 border-glass-border" required />
          </Field>
          ...
          <Field className="col-span-1 md:col-span-1" label="Category">
            <Select value={form.category} onValueChange={(v: TxCategory) => setForm({ ...form, category: v })}>
              <SelectTrigger className="bg-input/40 border-glass-border"><SelectValue /></SelectTrigger>
              <SelectContent>
          ...
          <Field className="col-span-1 md:col-span-1" label="Amount (₹)">
            <Input type="number" step="1" min="0" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })}
              className="bg-input/40 border-glass-border tabular-nums" placeholder="0" required />
          </Field>
          <Field className="col-span-2 md:col-span-3" label="Tags (comma-sep)">
            <Input value={form.tags} onChange={(e) => setForm({ ...form, tags: e.target.value })}
              className="bg-input/40 border-glass-border" placeholder="essentials, may" />
          </Field>
          <Field className="col-span-2 md:col-span-3" label="Notes">
            <Textarea rows={1} value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })}
              className="bg-input/40 border-glass-border min-h-[40px]" placeholder="Optional" />
          </Field>
          <div className="col-span-2 md:col-span-6 flex justify-end">
            <Button type="submit" className="gradient-primary text-primary-foreground border-0 gap-2 glow h-10">
      ...
      <section className="glass rounded-2xl p-5">
        <div className="flex flex-col md:flex-row gap-3 md:items-center md:justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{filtered.length} of {transactions.length} entries</p>
          </div>
          <div className="relative md:w-72">
            <Search className="size-4 absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" />
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search category, tag, note…"
        ...
        <div className="hidden md:block overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((t) => (
                <tr key={t.id} className="border-b border-glass-border/50 hover:bg-white/5 transition-colors">
                  <td className="py-3">{fmtDate(t.date)}</td>
                  <td>
                    <span className={`inline-flex items-center gap-1 text-xs px-2 py-1 rounded-md ${t.type === "income" ? "bg-[oklch(0.72_0.18_155/0.15)] text-[oklch(0.82_0.16_155)]" : "bg-[oklch(0.7_0.22_20/0.15)] text-[oklch(0.82_0.18_25)]"}`}>
                  ...
                  <td className="text-right">
                    <button onClick={() => handleDelete(t.id)} className="text-muted-foreground hover:text-destructive p-2">
              ...
              {filtered.length === 0 && (
                <tr><td colSpan={7} className="text-center py-8 text-muted-foreground">No entries match.</td></tr>
              )}
            </tbody>
          </table>
        </div>
          ...
          {filtered.map((t) => (
            <li key={t.id} className="glass rounded-xl p-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3 min-w-0">
                <div className={`size-9 rounded-xl grid place-items-center shrink-0 ${t.type === "income" ? "gradient-success" : "gradient-danger"}`}>
              ...
              <div className="text-right">
                <p className={`font-semibold tabular-nums text-sm ${t.type === "income" ? "text-[oklch(0.78_0.16_155)]" : "text-[oklch(0.78_0.18_25)]"}`}>
                  {t.type === "income" ? "+" : "−"}{inr(t.amount)}
                </p>
                <button onClick={() => handleDelete(t.id)} className="text-[11px] text-muted-foreground mt-1">delete</button>
              </div>
            </li>
          ))}
          {filtered.length === 0 && (

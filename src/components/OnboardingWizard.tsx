...
import { DEFAULT_INVESTMENT_APPS, DEFAULT_PAYMENT_MODES, useStore } from "@/lib/store";
...
// One-click suggestions — purely a head start. Anything typed into the free
// text field works too; these never block or gate the "real" defaults
// (Bank Account/Credit Card/UPI/Cash, and the 6 built-in broker partitions),
// which are already active for every account regardless of this wizard.
const BANK_SUGGESTIONS = ["HDFC Bank", "SBI", "ICICI Bank", "Axis Bank", "Paytm", "Google Pay"];
const BROKER_SUGGESTIONS = ["Zerodha", "Dhan", "Groww", "Upstox", "Angel One"];
 ...
 *
 * Closing it by ANY means (Skip, Finish, Escape, backdrop click) marks
 * onboarding complete: a half-dismissed wizard must never reappear uninvited
 * on the next navigation.
 */
export function OnboardingWizard() {
  const { hydrated, onboardingCompleted, isFirstTimeUser, completeOnboarding } = useStore();
...
// ─── Shared chip-picker step (accounts + brokers) ──────────────────────────
function ChipPickerStep({
  title,
  description,
  suggestions,
  added,
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {suggestions.map((s) => {
          const isAdded = addedLower.has(s.toLowerCase());
          return (
            <button
              key={s}
              type="button"
              onClick={() => (isAdded ? undefined : onAdd(s))}
              disabled={isAdded}
              className={cn(
                "flex items-center gap-1.5 text-sm px-3.5 py-2 rounded-full border transition-all",
                isAdded
                  ? "border-[oklch(0.72_0.18_155/0.4)] bg-[oklch(0.72_0.18_155/0.1)] text-[oklch(0.82_0.16_155)] cursor-default"
                  : "border-glass-border glass hover:border-primary/30 hover:bg-white/5",
              )}
            >
              {isAdded && <Check className="size-3.5" />} {s}
            </button>
          );
        })}
      </div>
...
function AccountsStep() {
  const { paymentModes, addAccountMode, deleteAccountMode } = useStore();
  return (
    <ChipPickerStep
      title="Bank accounts & credit cards"
      description="Bank Account, Credit Card, UPI, and Cash are already set up. Add the specific ones you use."
      suggestions={BANK_SUGGESTIONS}
      added={paymentModes.filter((m) => !DEFAULT_PAYMENT_MODES.includes(m))}
...
function PartitionsStep() {
  const { investmentApps, addPartition, deletePartition } = useStore();
  return (
    <ChipPickerStep
      title="Investment brokers & partitions"
      description="A starter set of partitions is already active. Name the actual brokers you trade or invest through."
      suggestions={BROKER_SUGGESTIONS}
      added={investmentApps
        .map((a) => a.id)
        .filter((id) => !DEFAULT_INVESTMENT_APPS.some((a) => a.id === id))}
